...
export default function PublicProjectModal({ isOpen, projectId, onClose }: PublicProjectModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-gray-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200 relative">
              ...
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-10">
                <div>
                  <div className="flex items-center gap-3 mb-3">
                    <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-900 tracking-tight break-words">{project.name}</h1>
                        ...
                        <div key={member.id} className="flex items-center p-4 bg-gray-50 rounded-2xl border border-gray-100 hover:border-brand/30 transition-colors min-w-0">
                          <div className="flex items-center gap-4 min-w-0 w-full">
                            ...
                            <div className="min-w-0 flex-1">
                              <p className="text-sm font-bold text-gray-900 truncate" title={member.user.name}>{member.user.name}</p>
