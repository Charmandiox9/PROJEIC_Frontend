...
export default function TabFinanzas({ projectId, subjectId, isLeader, isSupervisor, wallet, members, onRefresh }: TabFinanzasProps) {
  ...
  useEffect(() => {
    ...
    loadMyEarnings();
    loadCatalog();
  }, [projectId, subjectId, isLeader]);

  // 🔥 NUEVA FUNCIÓN PARA SUSCRIBIRSE/CONTRATAR
  const handleSubscribe = async (catalogItemId: string, price: number, name: string) => {
    ...
    try {
      ...
      onRefresh(); // Actualizamos la bóveda y el historial
    } catch (error: any) {
  ...
  if (!wallet) {
    return (
      <div className="p-8 text-center bg-surface-primary rounded-2xl border border-border-primary">
        <Landmark className="w-12 h-12 mx-auto text-text-muted mb-4" />
        <h3 className="text-lg font-bold">Bóveda no inicializada</h3>
        <p className="text-sm text-text-muted mt-1">Este proyecto aún no tiene una cuenta bancaria asignada.</p>
      </div>
    );
  }
              ...
              <div key={cost.id} className="bg-surface-secondary border border-border-primary rounded-2xl p-4 flex flex-col relative overflow-hidden group">
                {isLeader && (
                  <button 
                    ...
                    className="absolute top-2 right-2 text-text-muted hover:text-red-500 hover:bg-red-500/10 p-1.5 rounded-lg transition-colors z-20 opacity-0 group-hover:opacity-100 disabled:opacity-100"
            ...
            {isSupervisor && (
              <>
                <div className="bg-surface-primary p-6 rounded-2xl border border-border-primary shadow-sm">
                  ...
                  {/* ... (Tu form de fondeo actual) ... */}
                  <form onSubmit={handleFundProject} className="space-y-4">
                    <div>
                      <div className="relative">
                        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted font-bold">$</span>
                        <input type="number" min="1" value={fundAmount} onChange={(e) => setFundAmount(e.target.value)} className="w-full bg-surface-secondary border border-border-primary rounded-xl py-2.5 pl-8 pr-4 text-text-primary focus:ring-2 focus:ring-brand" placeholder="Ej: 50000" required />
                      </div>
                    </div>
                    <div>
                      <input type="text" value={fundDescription} onChange={(e) => setFundDescription(e.target.value)} className="w-full bg-surface-secondary border border-border-primary rounded-xl py-2.5 px-4 text-text-primary focus:ring-2 focus:ring-brand" placeholder="Descripción (Ej: Presupuesto inicial)" />
                    </div>
                ...
                <div className="bg-red-500/5 p-6 rounded-2xl border border-red-500/20 shadow-sm mt-6">
                  ...
                  {/* ... (Tu form de multas actual) ... */}
                  <form onSubmit={handleApplyPenalty} className="space-y-4">
                    <div>
                      <div className="relative">
                        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted font-bold">$</span>
                        <input type="number" min="1" value={penaltyAmount} onChange={(e) => setPenaltyAmount(e.target.value)} required className="w-full bg-surface-primary border border-border-primary rounded-xl py-2.5 pl-8 pr-4 text-text-primary focus:ring-2 focus:ring-red-500" placeholder="Ej: 10000" />
                      </div>
                    </div>
                    <div>
                      <input type="text" value={penaltyReason} onChange={(e) => setPenaltyReason(e.target.value)} required className="w-full bg-surface-primary border border-border-primary rounded-xl py-2.5 px-4 text-text-primary focus:ring-2 focus:ring-red-500" placeholder="Motivo (Ej: Bug crítico)" />
                    </div>
            ...
            {isLeader && (
              <>
                <div className="bg-surface-primary p-6 rounded-2xl border border-border-primary shadow-sm">
                  ...
                  {/* ... (Tu form de pagos actual) ... */}
                  <form onSubmit={handlePayMember} className="space-y-4">
                    <div>
                      <select value={payRecipient} onChange={(e) => setPayRecipient(e.target.value)} className="w-full bg-surface-secondary border border-border-primary rounded-xl py-2.5 px-4 text-text-primary focus:ring-2 focus:ring-brand" required>
                    ...
                    <div>
                      <div className="relative">
                        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted font-bold">$</span>
                        <input type="number" min="1" value={payAmount} onChange={(e) => setPayAmount(e.target.value)} className="w-full bg-surface-secondary border border-border-primary rounded-xl py-2.5 pl-8 pr-4 text-text-primary focus:ring-2 focus:ring-brand" placeholder="Ej: 15000" required />
                      </div>
                    </div>
                    <div>
                      <input type="text" value={payDescription} onChange={(e) => setPayDescription(e.target.value)} className="w-full bg-surface-secondary border border-border-primary rounded-xl py-2.5 px-4 text-text-primary focus:ring-2 focus:ring-brand" placeholder="Concepto (Ej: Pago Sprint 1)" />
                    </div>
                ...
                {/* 🔥 TIENDA DE RECURSOS FUNCIONAL */}
                <div className="bg-surface-primary p-6 rounded-2xl border border-border-primary shadow-sm mt-6">
                  <div className="flex items-center gap-2 mb-4">
                    <ShoppingCart className="w-5 h-5 text-purple-500" />
                    <h3 className="font-bold text-text-primary">Catálogo de Servicios</h3>
                  </div>
                  <p className="text-sm text-text-muted mb-4">
                    Adquiere recursos o infraestructura definida por tu profesor.
                  </p>
                  ...
                  <div className="space-y-3">
                    {isLoadingCatalog ? (
                       ...
                       <p className="text-xs text-center text-text-muted py-2">Este proyecto no está vinculado a una asignatura institucional.</p>
                    ) : catalogItems.length === 0 ? (
                       <div className="text-center py-4 flex flex-col items-center opacity-50">
                         <PackageOpen className="w-8 h-8 mb-2" />
                         <p className="text-xs text-text-muted">El profesor aún no ha añadido recursos.</p>
                       </div>
                    ) : (
                       catalogItems.map(item => (
                         <button 
                           ...
                           className="w-full flex justify-between items-center p-3 border border-border-primary rounded-xl hover:border-purple-500 hover:bg-purple-500/5 transition-all text-left disabled:opacity-50"
                         ...
                         >
                           ...
                           <span className="font-bold text-purple-500">-{formatCurrency(item.basePrice, wallet.currency)}</span>
                         </button>
                       ))
                    )}
                  </div>
