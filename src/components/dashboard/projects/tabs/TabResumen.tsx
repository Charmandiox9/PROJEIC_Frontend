    ...
    <div className="space-y-6">
        ...
        <div className="p-6 flex flex-col md:flex-row md:items-start justify-between gap-4">
          ...
          {isLeader && (
            <div className="flex items-center gap-2 shrink-0">
              ...
              >
                <Edit3 className="w-4 h-4" /> {t('projectDetail.edit')}
              </button>
              <button
                onClick={onDelete}
                disabled={isDeleting}
              ...
              >
                ...
                {isDeleting ? 'Eliminando...' : t('projectDetail.delete')}
              </button>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-6 space-y-4">
          <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest">{t('projectDetail.detailsTitle')}</h3>
          <ul className="space-y-3">
            <li className="flex items-center gap-3">
              <Calendar className="w-4 h-4 text-gray-400 shrink-0" />
              <span className="text-sm text-gray-500 dark:text-gray-400 min-w-[120px]">{t('projectDetail.createdOn')}</span>
            ...
            <li className="flex items-center gap-3">
              ...
              <span className="text-sm text-gray-500 dark:text-gray-400 min-w-[120px]">{t('projectDetail.methodology')}</span>
            ...
            {project.isInstitutional && project.subject && (
              <>
                <li className="flex items-start gap-3">
                  ...
                  <span className="text-sm text-gray-500 dark:text-gray-400 min-w-[120px]">{t('projectDetail.subject')}</span>
                  <div className="flex flex-col">
                    <span className="text-sm font-bold text-brand">
                      {project.subject.name} {project.subject.code ? `(${project.subject.code})` : ''}
                    </span>
                    <span className="text-xs text-gray-500">{t('projectDetail.period').replace('{p}', project.subject.period)}</span>
                  </div>
                </li>
                ...
                {project.subject.professors && project.subject.professors.length > 0 && (
                  <li className="flex items-start gap-3">
                    ...
                    <span className="text-sm text-gray-500 min-w-[120px]">{t('projectDetail.professors')}</span>
            ...
            <li className="flex items-center gap-3">
              ...
              <span className="text-sm text-gray-500 min-w-[120px]">{t('projectDetail.visibility')}</span>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            {isLeader && (
              ...
              >
                <UserPlus className="w-3.5 h-3.5" /> {t('projectDetail.addMember')}
              </button>
            )}
          </div>
          ...
          {activeMembers.length > 0 ? (
            <ul className="space-y-2">
              {activeMembers.map((member) => (
                <li key={member.id} className="flex items-center justify-between p-2.5 bg-gray-50 dark:bg-gray-700 rounded-lg border border-gray-100 dark:border-gray-600">
                    ...
                    <div className="min-w-0">
                      ...
                      {member.status === 'PENDING' && (
                        <span className="flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-amber-700 bg-amber-100 border border-amber-200 rounded-md">
                          <Loader2 className="w-3 h-3 animate-spin" /> {t('projectDetail.pending')}
                        </span>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    {isLeader ? (
                      <label className="relative cursor-pointer" title={t('projectDetail.changeRole')}>
            ...
            <div className="text-center py-8 bg-gray-50 dark:bg-gray-700 rounded-lg border border-dashed border-gray-200 dark:border-gray-600">
              ...
              <p className="text-xs text-gray-400">{t('projectDetail.noMembers')}</p>
            </div>
          )}
        </div>
      </div>
