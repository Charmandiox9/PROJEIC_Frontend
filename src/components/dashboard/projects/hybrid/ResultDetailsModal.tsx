  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm animate-in fade-in duration-200">
        <div className="bg-surface-primary rounded-2xl shadow-xl w-full max-w-3xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
          
          {/* HEADER */}
          <div className="px-6 py-5 border-b border-border-primary shrink-0 bg-surface-secondary/50">
            <div className="flex justify-between items-start gap-4">
              <div>
                ...
                <h2 className="text-xl font-bold text-text-primary leading-tight">{result.title}</h2>
                <p className="text-sm text-text-muted mt-2 line-clamp-2">{result.description}</p>
              </div>
              <button onClick={onClose} className="p-2 text-text-muted hover:text-text-primary hover:bg-surface-secondary rounded-full transition-colors shrink-0">
                <X className="w-5 h-5" />
              </button>
            </div>
          </div>
          ...
          {/* CONTENIDO DEL TAB */}
          <div className="p-6 overflow-y-auto custom-scrollbar flex-1 bg-surface-secondary/30">
              ...
              <div className="space-y-4">
                <form onSubmit={handleCreateTask} className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-4 py-2 text-sm border border-border-secondary rounded-lg focus:ring-2 focus:ring-brand focus:border-brand outline-none transition-shadow disabled:bg-surface-secondary"
                  />
                  <button 
                    type="submit" 
                ...
                {!result.tasks || result.tasks.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {result.tasks.map((task: any) => {
                      ...
                      return (
                        <div key={task.id} className="group flex items-center justify-between p-3 bg-surface-primary border border-border-primary rounded-lg shadow-sm hover:border-brand/50 transition-colors">
                          <div className="flex items-center gap-3 flex-1 cursor-pointer" onClick={() => !isLoading && handleToggleTask(task)}>
                            {isLoading ? (
                              ...
                              <input 
                                ...
                                checked={isDone} 
                                readOnly 
                                className="w-4 h-4 rounded border-border-secondary text-brand focus:ring-brand cursor-pointer" 
                              />
                            )}
                            <span className={`text-sm font-medium transition-all ${isDone ? 'text-text-muted line-through' : 'text-text-secondary'}`}>
                              {task.title}
                            </span>
                          </div>
                          
                          <div className="flex items-center gap-3 ml-4">
                            <span className="text-[10px] font-bold uppercase px-2 py-1 bg-surface-secondary rounded text-text-secondary">
                              {task.status}
                            </span>
                            <button 
                              onClick={() => handleDeleteTask(task.id)}
                              disabled={isLoading}
                              className="p-1.5 text-text-muted hover:text-red-500 hover:bg-red-50 rounded-md transition-colors opacity-0 group-hover:opacity-100 disabled:opacity-0"
                      ...
                      <div key={ev.id} className="p-4 bg-surface-primary border border-border-primary rounded-xl flex flex-col gap-3 hover:shadow-md transition-shadow">
                        ...
                        {ev.type === 'URL' ? (
                          ...
                          <div 
                            ...
                            className="relative w-full h-32 mt-2 rounded-lg border border-border-primary overflow-hidden cursor-zoom-in group bg-surface-secondary"
                          >
                            <img 
