...
import { X, Target, ListTodo, ShieldCheck, History, ExternalLink, Download, Plus, AlertCircle, Trash2, Loader2 } from 'lucide-react';
                    ...
                    <div key={ev.id} className="p-4 bg-white border border-gray-200 rounded-xl flex flex-col gap-3 hover:shadow-md transition-shadow">
                      ...
                      {ev.type === 'URL' ? (
                        ...
                        <a 
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            const url = getFileUrl(ev.fileKey);
                            console.log("Abriendo URL:", url);
                          }}
                          className="flex items-center gap-2 text-sm font-medium text-gray-700 hover:text-brand bg-gray-50 p-3 rounded-lg border border-gray-200 hover:border-brand/30 transition-colors"
                        >
                  ...
                  {result.history.map((log: any) => (
                    <div key={log.id} className="relative pl-6 group">
                      ...
                      <div className="bg-white p-4 border border-gray-100 rounded-xl shadow-sm hover:border-brand/30 transition-colors">
                        ...
                        {/* Si hubo un motivo de retroceso, lo mostramos como advertencia */}
                        {log.reason && (
