      ...
      <div className="grid grid-cols-1 gap-3">
        {artifacts?.map((art: any) => (
          <div key={art.id} className="p-3 bg-surface-secondary border border-border-secondary rounded-lg flex items-center justify-between">
            ...
            {art.expired ? (
              <span className="text-[10px] text-red-500 font-bold bg-red-500/10 px-2 py-1 rounded uppercase shrink-0 tracking-wider">Expirado</span>
            ) : (
              <button 
        ...
        {(!artifacts || artifacts.length === 0) && <p className="text-xs text-center text-text-muted p-2">No hay artefactos disponibles.</p>}
      </div>
    </div>
  );
}